...
if($Default.IsPresent)
{
 ...
 $StartDate= ((Get-Date).AddDays(-1825)).Date
}
...
While($true)
{
 ...
 Try
 {
  ...
  if($Date -gt ((Get-Date).AddDays(-1825)))
  { 
   break
  }
  else
  {
   Write-Host `nAudit log can be retrieved only for past 1825 days. Please select a date after (Get-Date).AddDays(-1825) -ForegroundColor Red
   return
  }
 }
 Catch
